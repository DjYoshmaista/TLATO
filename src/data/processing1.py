...
"""       
    def _process_text(self, filepath: Path):
        ...
        try:
            ...
            lines = content.splitlines()
            if not lines:
                logger.warning(f"Text file is empty: {filepath.name}")
                return pd.Series(dtype=str) # Return empty Series

            # Always use pandas for text processing steps for consistency
            # NLTK functions typically work on standard Python strings
            pdf = pd.Series(lines)
            processed = pdf.str.lower()
            # Keep basic punctuation that might be relevant ('-', '.')? Adjust regex as needed.
            processed = processed.str.replace(r'[^\w\s\-\.]', '', regex=True) # Keep word chars, space, hyphen, dot
            processed = processed.str.replace(r'\s+', ' ', regex=True).str.strip() # Consolidate whitespace

            # Apply lemmatization and stopword removal only if NLTK available
            if NLTK_AVAILABLE and lemmatizer:
                def lemmatize_and_filter(text):
                     words = text.split()
                     # Lemmatize first, then filter stopwords
                     lemmatized = [lemmatizer.lemmatize(w) for w in words]
                     filtered = [w for w in lemmatized if w not in stop_words]
                     return " ".join(filtered)

                processed = processed.apply(lemmatize_and_filter)
                logger.debug(f"NLTK text processing complete for {filepath.name} (CPU)")
            else:
                 logger.debug(f"Basic text processing complete for {filepath.name} (CPU, NLTK unavailable)")
                 # Return Series of cleaned lines if NLTK not used for further processing
                 return processed.dropna() # Drop empty lines after cleaning

            return processed.dropna() # Return pandas Series of processed lines/docs
    ...
    def _process_numerical(self, filepath: Path):
        # Processes numerical data from a JSON file, attempting JSON Lines format first,
        # then falling back to parsing specific structures. Returns a NumPy or CuPy array.

        logger.debug(f"Processing numerical file: {filepath.name}")
        processed_array = None # To store the final numpy/cupy array

        # --- Approach 1: Try reading as JSON Lines directly into DataFrame ---
        try:
            logger.debug(f"Attempting to read {filepath.name} as JSON Lines...")
            df = None
            if GPU_AVAILABLE and cudf is not None and not isinstance(cudf, type(cudf_dummy)): # Check if cudf is real
                try:
                    df = cudf.read_json(filepath, lines=True, dtype=False) # Try inferring dtype
                    logger.debug(f"Successfully read JSON Lines into cuDF DataFrame with {len(df)} rows.")
                    # Convert to numpy/cupy array for consistent processing below
                    # Heuristic: If single column, treat as 1D array. Otherwise, full matrix.
                    if len(df.columns) == 1:
                         num_array = df.iloc[:, 0].to_cupy() # Get as CuPy Series -> CuPy array
                    else:
                         num_array = df.to_cupy() # Get as CuPy array
                    df = None # Clear DataFrame from memory
                except Exception as cudf_read_err:
                    logger.warning(f"cuDF failed to read {filepath.name} as JSON Lines ({cudf_read_err}). Falling back.")
                    df = None # Ensure df is None
                    num_array = None # Ensure num_array is None
            else: # Fallback to pandas for reading
                 df = pd.read_json(filepath, lines=True, dtype=False) # Try inferring dtype
                 logger.debug(f"Successfully read JSON Lines into pandas DataFrame with {len(df)} rows.")
                 # Convert to numpy array for consistent processing below
                 if len(df.columns) == 1:
                      num_array = df.iloc[:, 0].to_numpy() # Get as NumPy Series -> NumPy array
                 else:
                      num_array = df.to_numpy() # Get as NumPy array
                 df = None # Clear DataFrame from memory

            # If reading succeeded and produced an array, proceed with scaling etc.
            if num_array is not None:
                # Ensure correct dtype (float32 is common)
                if num_array.dtype.kind not in ['f', 'i']: # Check if not float or integer
                    logger.warning(f"Array from JSONL read is not numeric (dtype: {num_array.dtype}). Attempting conversion to float32.")
                    try:
                        # Use cp.astype if it's a cupy array, otherwise numpy
                        num_array = num_array.astype(np.float32) if isinstance(num_array, np.ndarray) else cp.asarray(num_array).astype(cp.float32)
                    except (ValueError, TypeError) as e:
                         logger.error(f"Could not convert array from JSON Lines to float32: {e}. Falling back to structure parsing.")
                         num_array = None # Reset num_array to trigger fallback

        except ValueError as pd_json_err: # read_json raises ValueError on bad format
            logger.warning(f"Could not read {filepath.name} as JSON Lines ({pd_json_err}). Falling back to full parse.")
            num_array = None # Ensure num_array is None for fallback
        except Exception as read_err: # Catch other potential read errors
            logger.error(f"Error reading {filepath.name} as JSON Lines: {read_err}", exc_info=True)
            num_array = None # Ensure num_array is None for fallback

        # --- Approach 2: Fallback to full read and structure parsing if Approach 1 failed ---
        if num_array is None:
            logger.debug(f"Falling back to full JSON parse for {filepath.name}")
            try:
                content = self._read_content(filepath) # Assumes self._read_content exists
                data = json.loads(content)

                # Extract numerical data based on expected structures
                if isinstance(data, list):
                    num_array = np.array(data, dtype=np.float32)
                elif isinstance(data, dict) and 'values' in data and isinstance(data['values'], list):
                    num_array = np.array(data['values'], dtype=np.float32)
                elif isinstance(data, dict) and 'input' in data and isinstance(data['input'], list):
                    num_array = np.array(data['input'], dtype=np.float32)
                else:
                    # If Approach 1 failed AND this fails, then the file is problematic
                    raise ValueError(f"Loaded JSON data is not JSON Lines and not in a recognized structure (list, dict with 'values', or dict with 'input'). Type was: {type(data)}")

            except json.JSONDecodeError as json_e:
                logger.error(f"Invalid JSON in numerical file {filepath}: {json_e}", exc_info=False)
                self.repo.update_entry(filepath, status='error', error_message=f"Invalid JSON: {json_e}")
                return None
            except ValueError as val_err: # Catch the structure ValueError
                 logger.error(f"JSON structure error for {filepath}: {val_err}", exc_info=False)
                 self.repo.update_entry(filepath, status='error', error_message=str(val_err))
                 return None
            except Exception as parse_err: # Catch other errors (e.g., from _read_content)
                logger.error(f"Failed during full parse fallback for {filepath}: {parse_err}", exc_info=True)
                self.repo.update_entry(filepath, status='error', error_message=f"Full parse failed: {parse_err}")
                return None

        # --- Process the extracted numerical array (common logic) ---
        try:
            if num_array is None or num_array.size == 0: return cp.array([]) if GPU_AVAILABLE else np.array([]) # Return empty
            if num_array.dtype.kind not in ['f', 'i']: num_array = num_array.astype(np.float32)
            if GPU_AVAILABLE and cp is not np and isinstance(num_array, np.ndarray): num_array = cp.asarray(num_array) # Move to GPU
            processed_array = num_array # Start with potentially GPU array
            logger.debug(f"Initial numerical array shape: {num_array.shape}, dtype: {num_array.dtype}")
            # Apply scaling if a scaler is available
            if self.scaler:
                logger.debug(f"Applying scaler ({type(self.scaler).__name__}) to data.")
                # Reshape if necessary (scaler expects 2D)
                original_ndim = num_array.ndim
                if original_ndim == 1:
                    array_2d = num_array.reshape(-1, 1)
                elif original_ndim > 1:
                    array_2d = num_array # Assume already suitable shape
                else: # 0-dim array?
                     logger.warning(f"Cannot scale 0-dimensional array from {filepath.name}")
                     return num_array # Return unscaled
                try:
                    # Use the scaler (could be cuML or Sklearn wrapper)
                    scaled_array_2d = self.scaler.fit_transform(array_2d)
                    # Reshape back if originally 1D
                    if original_ndim == 1:
                        processed_array = scaled_array_2d.reshape(-1)
                    else:
                        processed_array = scaled_array_2d
                    logger.debug(f"Numerical scaling complete for {filepath.name}.")
                except Exception as scale_err:
                    # If scaling fails, log error and return the *unscaled* array
                    logger.error(f"Scaling failed for {filepath.name}: {scale_err}. Returning unscaled array.", exc_info=True)
                    processed_array = num_array # Fallback to unscaled array
            else:
                # No scaler available, use the array as is
                logger.debug(f"No scaler configured. Using unscaled numerical data for {filepath.name}.")
                processed_array = num_array
            # Final check type and return
            logger.debug(f"Final processed array type: {type(processed_array)}, shape: {processed_array.shape}")
            return processed_array
        except Exception as final_proc_err:
            logger.error(f"Final processing/scaling failed for {filepath.name}: {final_proc_err}", exc_info=True)
            self.repo.update_entry(filepath, status='error', error_message=f"Final processing failed: {final_proc_err}")
            return None

    def _process_pdf(self, filepath: Path):
        # Processes PDF files. Returns the processed data.
        logger.debug(f"Processing PDF file: {filepath.name}")
        try:
            # Use PyMuPDF or pdfminer for PDF processing
            # Placeholder for actual implementation
            content = self._read_content(filepath)
            return pd.Series(content.splitlines())
        except Exception as e:
            logger.error(f"Failed to process PDF file {filepath}: {e}", exc_info=True)
            return None
        except FileNotFoundError:
            logger.error(f"File not found during PDF processing: {filepath}", exc_info=True)
            return None
        
    def _save_processed(self, data, original_path: Path) -> Path | None:
        # Saves processed data with configured zstandard compression.
        if data is None: return None
        ...
        else: logger.error(f"Unsupported data type for saving: {type(data)}"); return None

        # Add .zst extension if compression is enabled
        if COMPRESSION_ENABLED: suffix += ".zst"

        save_path = PROCESSED_DATA_DIR / f"{save_stem}_processed{suffix}"
        try:
            save_path.parent.mkdir(parents=True, exist_ok=True)
            logger.debug(f"Saving processed data for '{original_path.name}' to '{save_path.name}'")

            # Prepare data if needed (e.g., CuPy to NumPy)
            data_to_save = data
            if cp is not np and isinstance(data, cp.ndarray):
                logger.debug("Converting CuPy array to NumPy for saving.")
                data_to_save = cp.asnumpy(data)
            ...
            # Write logic
            if COMPRESSION_ENABLED:
                cctx = zstd.ZstdCompressor(level=COMPRESSION_LEVEL) # Use configured level
                with open(save_path, 'wb') as f:
                    with cctx.stream_writer(f) as compressor:
                        if is_numpy: np.save(compressor, data_to_save, allow_pickle=False)
                        else: compressor.write(data_to_save.to_csv(index=False, header=isinstance(data_to_save, pd.DataFrame)).encode('utf-8'))
            else: # Save uncompressed
                 if is_numpy: np.save(save_path, data_to_save, allow_pickle=False)
                 else: data_to_save.to_csv(save_path, index=False, header=isinstance(data_to_save, pd.DataFrame))

            logger.info(f"Successfully saved processed file: {save_path}")
            return save_path
        except Exception as e:
