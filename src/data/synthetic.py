...
try:
    ...
    log_statement(loglevel=str("info"), logstatement=str("Logger for Synthetic Data Generation module initialized."), main_logger=str(__name__))
    log_statement(loglevel=str("info"), logstatement=str("Synthetic data generation module initialized."), main_logger=str(__name__))
except ImportError:
    ...
    try:
        ...
        log_statement(loglevel=str("error"), logstatement=str("Failed all imports for config/logger. Defining dummy config/logger."), main_logger=str(__name__))
        class DummyConfig:
...
class SyntheticDataGenerator:
    ...
    def __init__(self):
        ...
        log_statement(loglevel=str("info"), logstatement=str(f"SyntheticDataGenerator initialized: OutputDir='{self.output_dir}', AdjustedOutputDir='{self.adjusted_output_dir}', TargetSamples={self.target_samples}, BatchSize={self.batch_size}"), main_logger=str(__name__))
        log_statement(loglevel=str("info"), logstatement=str(f"Using Ollama Endpoint: {self.ollama_endpoint}"), main_logger=str(__name__))
        log_statement(loglevel=str("info"), logstatement=str(f"Generation Model: {self.generation_model}, Correction Model: {self.correction_model}"), main_logger=str(__name__))
        log_statement(loglevel=str("info"), logstatement=str(f"API Correction Enabled: {self.enable_api_correction}"), main_logger=str(__name__))
        log_statement(loglevel=str("info"), logstatement=str(f"Save retries: {self.save_retries}, API Timeout: {self.api_timeout}s"), main_logger=str(__name__))
    ...
    def _call_ollama_api(self, prompt: str, context: str, model_name: str, retries: int = 1) -> str | None:
        ...
        log_statement(loglevel=str("debug"), logstatement=str(f"API Call [{context}] - Model: {model_name}, Prompt: {prompt[:150]}..."), main_logger=str(__name__))
            ...
            try:
                ...
                try:
                    ...
                    if 'response' in response_data:
                         ...
                         log_statement(loglevel=str("warning"), logstatement=str(f"Unexpected JSON structure in API response [{context}]: {response_data}. Using raw text."), main_logger=str(__name__))
                ...
                log_statement(loglevel=str("warning"), logstatement=str(f"API request [{context}] timed out ({self.api_timeout}s). Attempt {attempt + 1}/{retries + 1}"), main_logger=str(__name__))
                ...
                log_statement(loglevel=str("warning"), logstatement=str(f"API request [{context}] failed: {e}. Attempt {attempt + 1}/{retries + 1}. Response: {response_text}..."), main_logger=str(__name__))
            except Exception as e:
                # Catch any other unexpected errors during the request
                log_statement(loglevel=str("error"), logstatement=str(f"Unexpected error during API call [{context}]: {e}. Attempt {attempt + 1}/{retries + 1}", exc_info=True), main_logger=str(__name__))
        ...
        log_statement(loglevel=str("error"), logstatement=str(f"API call [{context}] failed after {retries + 1} attempts."), main_logger=str(__name__))
        return None
    ...
    def _try_parse_json(self, json_string: str, context: str) -> dict | list | None:
        ...
        if not json_string or not isinstance(json_string, str):
             log_statement(loglevel=str("warning"), logstatement=str(f"Invalid input for JSON parsing ({context}): Type {type(json_string)}. Content: '{str(json_string)[:100]}...'"), main_logger=str(__name__))
             return None
        try:
            # --- Cleanup ---
            ...
            if start != -1 and end != -1 and end > start:
                ...
                log_statement(loglevel=str("warning"), logstatement=str(f"Could not find valid JSON object/array markers in response ({context}): {processed_string[:200]}..."), main_logger=str(__name__))
            ...
            log_statement(loglevel=str("warning"), logstatement=str(f"JSON decode error during parsing ({context}): {e}. String was: '{log_content}'"), main_logger=str(__name__))
            return None
        except Exception as e:
            # Catch any other unexpected errors during cleanup or parsing
            log_statement(loglevel=str("error"), logstatement=str(f"Unexpected error during JSON parsing ({context}): {e}", exc_info=True), main_logger=str(__name__))
            return None
    ...
    def _correct_json_formatting_via_api(self, bad_json_string: str) -> dict | list | None:
        ...
        if not self.enable_api_correction:
            log_statement(loglevel=str("debug"), logstatement=str("API correction disabled, skipping JSON format correction."), main_logger=str(__name__))
            return None

        log_statement(loglevel=str("warning"), logstatement=str("Attempting JSON correction via API..."), main_logger=str(__name__))
        ...
        if corrected_text:
            ...
            if parsed_result is not None: # Check if parsing succeeded (could be dict or list)
                log_statement(loglevel=str("info"), logstatement=str("Successfully corrected JSON format via API."), main_logger=str(__name__))
                return parsed_result
            else:
                log_statement(loglevel=str("error"), logstatement=str(f"API correction returned text that still couldn't be parsed as JSON: {corrected_text[:500]}..."), main_logger=str(__name__))
        else:
            # Log if the API call itself failed
            log_statement(loglevel=str("error"), logstatement=str("API call for JSON correction failed."), main_logger=str(__name__))
    ...
    def _correct_list_via_api(self, current_list: list | None, list_name: str, expected_len: int, value_range: tuple[float, float]) -> list | None:
        ...
        if not self.enable_api_correction:
            log_statement(loglevel=str("debug"), logstatement=str(f"API correction disabled. Applying local correction for '{list_name}'."), main_logger=str(__name__))
        ...
        if is_length_wrong and not has_bad_types:
             ...
             log_statement(loglevel=str("debug"), logstatement=str(f"List '{list_name}' requires no API correction."), main_logger=str(__name__))
             # Convert ints to float if present
             return [float(item) for item in current_list]

        # --- Call API for Correction ---
        if correction_needed:
            log_statement(loglevel=str("warning"), logstatement=str(f"Attempting {context} via API..."), main_logger=str(__name__))
            ...
            if corrected_text:
                ...
                if isinstance(parsed_list, list):
                    ...
                    if len(parsed_list) == expected_len and all(isinstance(item, (int, float)) for item in parsed_list):
                        log_statement(loglevel=str("info"), logstatement=str(f"Successfully corrected list '{list_name}' via API."), main_logger=str(__name__))
                        # Ensure all are floats
                        return [float(item) for item in parsed_list]
                    else:
                        # Log error if API correction resulted in unexpected format
                        log_statement(loglevel=str("error"), logstatement=str(f"API correction for '{list_name}' resulted in a list with incorrect length ({len(parsed_list)} has, {expected_len} needed) or non-float types. Applying local correction as fallback."), main_logger=str(__name__))
                        # Fall through to local correction
                else:
                     # Log error if API response wasn't a list
                     log_statement(loglevel=str("error"), logstatement=str(f"API correction for '{list_name}' did not return a valid JSON list: {corrected_text[:500]}... Applying local correction."), main_logger=str(__name__))
                     # Fall through to local correction
            else:
                # Log error if API call failed
                log_statement(loglevel=str("error"), logstatement=str(f"API call for list correction ('{list_name}') failed. Applying local correction."), main_logger=str(__name__))
                # Fall through to local correction
        else:
    ...
    def _apply_local_list_correction(self, current_list: list | None, expected_len: int, value_range: tuple[float, float], list_name: str) -> list:
        ...
        log_statement(loglevel=str("debug"), logstatement=str(f"Applying local correction to list '{list_name}'."), main_logger=str(__name__))
        if current_list is None: current_list = []
        ...
        if current_len > expected_len:
            log_statement(loglevel=str("warning"), logstatement=str(f"Locally correcting '{list_name}': Truncating list from {current_len} to {expected_len}."), main_logger=str(__name__))
            corrected_list = corrected_list[:expected_len]
        elif current_len < expected_len:
            padding_needed = expected_len - current_len
            log_statement(loglevel=str("warning"), logstatement=str(f"Locally correcting '{list_name}': Padding list with {padding_needed} value(s) ({PAD_VALUE})."), main_logger=str(__name__))
    ...
    def _generate_and_correct_sample(self) -> dict | None:
        ...
        if not raw_response_text:
            log_statement(loglevel=str("error"), logstatement=str("Failed to get any response from generation API after retries."), main_logger=str(__name__))
            return None
        ...
        if not isinstance(parsed_sample, dict): # Check if it's a dictionary specifically
            log_statement(loglevel=str("warning"), logstatement=str("Initial generation response was not parsed as a valid dictionary."), main_logger=str(__name__))
            ...
            if not isinstance(parsed_sample, dict):
                log_statement(loglevel=str("error"), logstatement=str("Failed to parse or correct response into a dictionary structure."), main_logger=str(__name__))
        ...
        if 'input' not in parsed_sample or 'target' not in parsed_sample:
             log_statement(loglevel=str("error"), logstatement=str(f"Essential key 'input' or 'target' missing after parsing/correction. Keys present: {list(parsed_sample.keys())}"), main_logger=str(__name__))
             return None
        ...
        if corrected_input_list is None: # Check if correction failed badly (should return list or fallback)
             log_statement(loglevel=str("error"), logstatement=str("Failed to correct 'input' list structure even with fallback."), main_logger=str(__name__))
        ...
        if corrected_target_list is None:
             log_statement(loglevel=str("error"), logstatement=str("Failed to correct 'target' list structure even with fallback."), main_logger=str(__name__))
        ...
        if adjusted_sample is None:
            log_statement(loglevel=str("error"), logstatement=str("Failed during final target normalization step."), main_logger=str(__name__))
            return None # Normalization failure is critical

        log_statement(loglevel=str("debug"), logstatement=str("Successfully generated, corrected, and adjusted sample structure."), main_logger=str(__name__))
        ...
        if isinstance(data, list):
            return data
             ...
             if data.strip().startswith('[') and data.strip().endswith(']'):
                 log_statement(loglevel=str("warning"), logstatement=str(f"Value for '{key_name}' is a string. Attempting basic parse."), main_logger=str(__name__))
                 parsed = self._try_parse_json(data, f"secondary parse of '{key_name}'")
                 if isinstance(parsed, list):
                     return parsed
                 else:
                      log_statement(loglevel=str("warning"), logstatement=str(f"Secondary parse of string for '{key_name}' failed."), main_logger=str(__name__))
                      return None if allow_correction else [] # Return None for correction, else empty list
             else:
                 log_statement(loglevel=str("warning"), logstatement=str(f"Value for '{key_name}' is a string but doesn't look like a list: {data[:50]}..."), main_logger=str(__name__))
                 return None if allow_correction else []
        # If not list or parsable string
        log_statement(loglevel=str("warning"), logstatement=str(f"Value for '{key_name}' is not a list or suitable string (type={type(data)})."), main_logger=str(__name__))
        return None if allow_correction else []
    ...
    def _normalize_target(self, sample_dict: dict) -> dict | None:
        ...
        try:
            ...
            if not isinstance(target_list, list):
                 log_statement(loglevel=str("error"), logstatement=str(f"Cannot normalize target: value is not a list (type: {type(target_list)}). Correction likely failed."), main_logger=str(__name__))
                 return None
            if len(target_list) != EXPECTED_TARGET_LEN:
                 log_statement(loglevel=str("error"), logstatement=str(f"Cannot normalize target: length is {len(target_list)}, expected {EXPECTED_TARGET_LEN}. Correction likely failed."), main_logger=str(__name__))
            ...
            for i, item in enumerate(target_list):
                 if isinstance(item, (int, float)):
                     ...
                     log_statement(loglevel=str("warning"), logstatement=str(f"Non-numeric item '{item}' found at index {i} in target list during normalization (post-correction). Replacing with {PAD_VALUE}."), main_logger=str(__name__))
                     numeric_target_list.append(PAD_VALUE)

            # --- Normalization Logic ---
            target_sum = sum(numeric_target_list)
            if abs(target_sum) < 1e-9: # Handle sum close to zero
                log_statement(loglevel=str("warning"), logstatement=str("Target list sum is zero after clamping/replacement. Setting uniform distribution."), main_logger=str(__name__))
                ...
                log_statement(loglevel=str("debug"), logstatement=str(f"Normalizing 'target' list. Original sum after clamping: {target_sum:.4f}"), main_logger=str(__name__))
                # Normalize
                ...
                if abs(final_sum - 1.0) > 1e-5 and final_sum > 1e-9:
                     log_statement(loglevel=str("debug"), logstatement=str(f"Re-normalizing target list after clamping. Sum was: {final_sum:.4f}"), main_logger=str(__name__))
            ...
            log_statement(loglevel=str("debug"), logstatement=str("Target normalization successful."), main_logger=str(__name__))
            return sample_dict

        except Exception as e:
             # Catch any unexpected errors during normalization
             log_statement(loglevel=str("error"), logstatement=str(f"Unexpected error during target normalization: {e}", exc_info=True), main_logger=str(__name__))
             return None
    ...
    def _attempt_save_adjusted_sample(self, sample_dict: dict, batch_index: int, sample_index_in_batch: int) -> bool:
        ...
        if not sample_dict or not isinstance(sample_dict, dict):
            log_statement(loglevel=str("warning"), logstatement=str(f"Attempted to save an invalid sample dict (empty or wrong type) for batch {batch_index}, sample {sample_index_in_batch}. Skipping."), main_logger=str(__name__))
            return False
        ...
        while attempts < max_attempts:
            try:
                ...
                log_statement(loglevel=str("debug"), logstatement=str(f"Successfully saved adjusted sample to {filename.name}"), main_logger=str(__name__))
                return True # Save successful

            # --- Exception Handling for Saving ---
            except (IOError, OSError) as io_err:
                # Handle file system errors (permissions, disk full, etc.)
                log_statement(loglevel=str("warning"), logstatement=str(f"Attempt {attempts + 1}/{max_attempts}: I/O error saving adjusted sample {filename.name}: {io_err}"), main_logger=str(__name__))
                ...
                log_statement(loglevel=str("error"), logstatement=str(f"Attempt {attempts + 1}/{max_attempts}: Type error saving adjusted sample {filename.name} (data not JSON serializable?): {type_err}", exc_info=False), main_logger=str(__name__))
                ...
                log_statement(loglevel=str("error"), logstatement=str(f"Attempt {attempts + 1}/{max_attempts}: Unexpected error saving adjusted sample {filename.name}: {e}", exc_info=True), main_logger=str(__name__))

            # --- Retry Logic ---
            attempts += 1
            if attempts < max_attempts:
                log_statement(loglevel=str("info"), logstatement=str(f"Retrying save for {filename.name} in {self.retry_delay} seconds..."), main_logger=str(__name__))
                time.sleep(self.retry_delay)
            else:
                # Log failure after all retries
                log_statement(loglevel=str("error"), logstatement=str(f"Failed to save adjusted sample {filename.name} after {max_attempts} attempts."), main_logger=str(__name__))
    ...
    def _generate_and_save_batch(self, batch_size: int, batch_index: int):
        ...
        """
        log_statement(loglevel=str("debug"), logstatement=str(f"Generating and saving batch {batch_index} (target size {batch_size})..."), main_logger=str(__name__))
        ...
        for i in range(batch_size):
             ...
             if self.successfully_saved_count >= self.target_samples:
                 log_statement(loglevel=str("info"), logstatement=str(f"Target sample count ({self.target_samples}) reached during batch {batch_index}. Stopping batch early."), main_logger=str(__name__))
                 break

             # Increment count for each generation *attempt*
             self.generated_samples_count += 1
             log_statement(loglevel=str("info"), logstatement=str(f"Attempting generation/correction for sample {self.generated_samples_count}/{self.target_samples} (Batch {batch_index}, Item {i})..."), main_logger=str(__name__))
             ...
             if corrected_sample:
                 ...
                 if save_successful:
                     ...
                     log_statement(loglevel=str("error"), logstatement=str(f"Failed to save processed sample {i} for batch {batch_index} after retries."), main_logger=str(__name__))
             else:
                  # Log if the generation/correction cycle failed for this sample
                  log_statement(loglevel=str("error"), logstatement=str(f"Failed to generate/correct sample {i} for batch {batch_index}."), main_logger=str(__name__))

        # Log completion of the batch
        log_statement(loglevel=str("debug"), logstatement=str(f"Finished batch {batch_index}. Samples successfully saved in this batch: {samples_saved_in_batch}/{batch_size}"), main_logger=str(__name__))
    ...
    def generate_dataset(self):
        ...
        if self.batch_size <= 0:
            log_statement(loglevel=str("error"), logstatement=str("Batch size must be positive. Aborting generation."), main_logger=str(__name__))
            return
        if self.target_samples <= 0:
            log_statement(loglevel=str("info"), logstatement=str("Target samples is zero or negative. No data generation needed."), main_logger=str(__name__))
            return

        # Calculate total number of batches needed
        total_batches = (self.target_samples + self.batch_size - 1) // self.batch_size
        log_statement(loglevel=str("info"), logstatement=str(f"Starting synthetic data generation for {self.target_samples} target samples in {total_batches} batches."), main_logger=str(__name__))
        ...
        with tqdm(total=self.target_samples, desc="Saving Samples", unit="sample") as pbar:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    log_statement(loglevel=str("error"), logstatement=str(f"Error processing a batch generation future: {e}", exc_info=True), main_logger=str(__name__))
                ...
                if current_saves >= self.target_samples:
                    log_statement(loglevel=str("info"), logstatement=str("Target sample count met or exceeded based on completed futures."), main_logger=str(__name__))
        ...
        log_statement(loglevel=str("info"), logstatement=str(f"Synthetic data generation finished. Target: {self.target_samples}, Actual Successfully Saved: {self.successfully_saved_count}"), main_logger=str(__name__))
        log_statement(loglevel=str("info"), logstatement=str(f"Total generation attempts (including failures/corrections): {self.generated_samples_count}"), main_logger=str(__name__))
...
#     log_statement(loglevel=str("info"), logstatement=str("Direct execution test finished.")
    ...
    def _generate_valid_sample(self, retries=3, timeout=90000):
        ...
        if current_retry == 0:
             log_statement(loglevel=str("info"), logstatement=str(f"Prompt for API: {prompt_text[:200]}...."), main_logger=str(__name__))
        ...
        while current_retry <= retries:
            ...
            if current_retry == 0:
                log_statement(loglevel=str("debug"), logstatement=str(f"Payload for API request (Attempt {current_retry+1}): {json.dumps(payload)}"), main_logger=str(__name__))
            ...
            try:
                ...
                if not generated_content_str:
                    log_statement(loglevel=str("warning"), logstatement=str(f"API response key 'response' is empty or missing. Response dict: {response_data}. Retrying ({current_retry}/{retries})..."), main_logger=str(__name__))
                    current_retry += 1
                    time.sleep(1 * (current_retry + 1))
                    continue

                # --- Optional: Log the raw response for debugging ---
                log_statement(loglevel=str("debug"), logstatement=str(f"Raw response content string received:\n{generated_content_str}"), main_logger=str(__name__))
                # --- End Optional ---

                sample = self._validate_sample(generated_content_str)
                if sample:
                    log_statement(loglevel=str("debug"), logstatement=str("Successfully generated and validated sample."), main_logger=str(__name__))
                    return sample
                else:
                    # Validation failed - _validate_sample logs details including JSONDecodeError
                    log_statement(loglevel=str("warning"), logstatement=str(f"API response content failed validation (see previous log). Retrying ({current_retry}/{retries})..."), main_logger=str(__name__))
                    current_retry += 1
                    time.sleep(1 * (current_retry + 1))

            except requests.exceptions.Timeout:
                log_statement(loglevel=str("warning"), logstatement=str(f"API request timed out ({timeout}s). Retrying ({current_retry}/{retries})..."), main_logger=str(__name__))
                current_retry += 1
                time.sleep(2 * (current_retry + 1))
            except requests.exceptions.RequestException as e:
                response_text = response.text if response else "NO RESPONSE"
                log_statement(loglevel=str("error"), logstatement=str(f"API request failed: {e}. Response: {response_text[:500]}... Retrying ({current_retry}/{retries})...", exc_info=False), main_logger=str(__name__))
                current_retry += 1
                time.sleep(2 * (current_retry + 1))
            except json.JSONDecodeError as json_err:
                response_text = response.text if response else "NO RESPONSE"
                log_statement(loglevel=str("error"), logstatement=str(f"Failed to decode overall API response as JSON: {json_err}. Response text: '{response_text[:500]}...'. Retrying ({current_retry}/{retries})...", exc_info=False), main_logger=str(__name__))
                current_retry += 1
                time.sleep(1 * (current_retry + 1))
            except Exception as e:
                log_statement(loglevel=str("error"), logstatement=str(f"Unexpected error during sample generation: {e}. Retrying ({current_retry}/{retries})...", exc_info=True), main_logger=str(__name__))
                current_retry += 1
                time.sleep(1 * (current_retry + 1))

        log_statement(loglevel=str("error"), logstatement=str(f"Failed to generate valid sample after {retries} retries."), main_logger=str(__name__))
        return None
    ...
    def _validate_sample(self, response_content: str) -> dict | None:
        ...
        try:
            ...
            if 'input' not in sample or 'target' not in sample:
                log_statement(loglevel=str("warning"), logstatement=str("Validation failed: Missing 'input' or 'target' key."), main_logger=str(__name__))
                return None
            ...
            if isinstance(input_data, str):
                log_statement(loglevel=str("debug"), logstatement=str("Detected 'input' value is a string. Attempting secondary parse..."), main_logger=str(__name__))
                try:
                    ...
                    log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'input' was a string but failed secondary parse: {e}. String was: '{input_data[:100]}...'"), main_logger=str(__name__))
                    return None

            # --- Check if target_data is a string and try parsing it ---
            if isinstance(target_data, str):
                log_statement(loglevel=str("debug"), logstatement=str("Detected 'target' value is a string. Attempting secondary parse..."), main_logger=str(__name__))
                try:
                    ...
                    log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'target' was a string but failed secondary parse: {e}. String was: '{target_data[:100]}...'"), main_logger=str(__name__))
                    return None
            ...
            if not isinstance(input_data, list):
                 ...
                 log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'input' is not a list (type={type(input_data)})."), main_logger=str(__name__))
                 return None
            if len(input_data) != 128:
                log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'input' list length is not 128 (len={len(input_data)})."), main_logger=str(__name__))
                return None
            ...
            if not all(isinstance(x, (int, float)) and -1.0 <= x <= 1.0 for x in input_data):
                ...
                log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'input' values not all floats/ints between -1.0 and 1.0. First invalid: {first_invalid_input}"), main_logger=str(__name__))
                return None

            # Validate target structure and constraints
            if not isinstance(target_data, list):
                log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'target' is not a list (type={type(target_data)})."), main_logger=str(__name__))
                return None
            if len(target_data) != 10:
                log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'target' list length is not 10 (len={len(target_data)})."), main_logger=str(__name__))
                return None
            ...
            if not all(isinstance(x, (int, float)) and 0.0 <= x <= 1.0 for x in target_data):
                 ...
                 log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'target' values not all floats/ints between 0.0 and 1.0. First invalid: {first_invalid_target}"), main_logger=str(__name__))
                 return None

            # Check sum with tolerance
            target_sum = sum(target_data)
            if not abs(target_sum - 1.0) < 1e-5:
                log_statement(loglevel=str("warning"), logstatement=str(f"Validation failed: 'target' values do not sum to 1.0 (sum={target_sum})."), main_logger=str(__name__))
                return None
            # --- End of structural/content validation logic ---

            log_statement(loglevel=str("debug"), logstatement=str("Validation successful."), main_logger=str(__name__))
            ...
            log_statement(loglevel=str("error"), logstatement=str(f"JSON decode error during validation: {e}. String was: '{log_content}'"), main_logger=str(__name__))
            return None
        except TypeError as e:
            log_statement(loglevel=str("error"), logstatement=str(f"Type error during validation (likely incorrect data structure): {e}"), main_logger=str(__name__))
            return None
        except Exception as e:
            log_statement(loglevel=str("error"), logstatement=str(f"Unexpected error during validation: {e}", exc_info=True), main_logger=str(__name__))
            return None
        finally:
            time.sleep(0.1)
    ...
    def _save_batch(self, batch_data: list, batch_index: int):
        ...
        try:
            ...
            log_statement(loglevel=str("info"), logstatement=str(f"Successfully saved batch {batch_index} ({len(batch_data)} samples) to {filename.name}"), main_logger=str(__name__))
            self.generated_samples_count += len(batch_data) # Increment successful count
        except IOError as e:
            log_statement(loglevel=str("error"), logstatement=str(f"Failed to write batch file {filename.name}: {e}", exc_info=True), main_logger=str(__name__))
        except Exception as e:
            log_statement(loglevel=str("error"), logstatement=str(f"Unexpected error saving batch {filename.name}: {e}", exc_info=True), main_logger=str(__name__))