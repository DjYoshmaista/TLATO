...
"""
    ...
    def process_all(self, base_dir_filter: Optional[Path] = None, statuses_to_process=('discovered', 'error', STATUS_NEW)):
        """Processes files matching status with enhanced progress tracking"""
        ...
        if not files_to_process_paths:
            log_statement('info', f"{LOG_INS}:INFO>>PROCESS_ALL: No files matching status {statuses_to_process} found to process.", Path(__file__).stem)
            return []
        ...
        try:
            ...
            log_statement('error', f"{LOG_INS}:ERROR>>PROCESS_ALL: Failed to retrieve file info: {e}", Path(__file__).stem, True)
            return []
        
        if not file_info_list:
            log_statement("warning", f"{LOG_INS}::PROCESS_ALL: Could not retrieve details for files.", Path(__file__).stem)
            return []
        ...
        for f_info in file_info_list:
            future = self.executor.submit(self._process_file, f_info)
            futures.append((future, f_info))
        
        # Process results with enhanced tracking
        successfully_processed_files_info = []
        
        for future, f_info in futures:
            try:
                result_info = future.result(timeout=300)  # 5 minute timeout per file
                file_name = Path(f_info.get(COL_FILEPATH, "Unknown")).name
                ...
                if result_info and isinstance(result_info, dict):
                    ...
                    if final_status == STATUS_PROCESSED:
                        successfully_processed_files_info.append(result_info)
                        progress_tracker.update(success=True)
                        log_statement('debug', f"{LOG_INS}:DEBUG>>Successfully processed: {file_name}", Path(__file__).stem)
                    else:
                        error_msg = result_info.get(COL_ERROR, "Unknown error")
                        progress_tracker.update(success=False, error_msg=f"{file_name}: {error_msg}")
                        log_statement('warning', f"{LOG_INS}:WARNING>>Failed to process {file_name}: {error_msg}", Path(__file__).stem)
                else:
                    progress_tracker.update(success=False, error_msg=f"{file_name}: Invalid result")
                    
            except Exception as e:
                file_name = Path(f_info.get(COL_FILEPATH, "Unknown")).name
                progress_tracker.update(success=False, error_msg=f"{file_name}: {str(e)}")
                log_statement('error', f"{LOG_INS}:ERROR>>Exception processing {file_name}: {e}", Path(__file__).stem, True)
        ...
        log_statement('info', f"{LOG_INS}:INFO>>PROCESS_ALL: Completed. Statistics: {stats}", Path(__file__).stem)
        
        # Save repository
        self.repo.save()
        
        return successfully_processed_files_info
    ...
    def add_folder(self, folder_path: str):
        """Scans folder with enhanced progress tracking"""
        LOG_INS = f"{__name__}::{self.__class__.__name__}::{inspect.currentframe().f_code.co_name}::{inspect.currentframe().f_lineno}"
        
        try:
            abs_folder_path = Path(folder_path).resolve()
            if not abs_folder_path.is_dir():
                log_statement('error', f"{LOG_INS}:ERROR>>Not a directory: {abs_folder_path}", Path(__file__).stem)
                return
        except Exception as e:
            log_statement('error', f"{LOG_INS}:ERROR>>Error resolving path: {e}", Path(__file__).stem, True)
            return
        ...
        total_files = sum(1 for _ in abs_folder_path.rglob('*') if _.is_file())
        ...
        progress_tracker = create_progress_tracker(
            ...
            description=f"Scanning {abs_folder_path.name}",
            unit="files",
            show_resources=True
        )
        ...
        try:
            ...
            log_statement('error', f"{LOG_INS}:ERROR>>Failed to load existing data: {e}", Path(__file__).stem, True)
            existing_paths = set()
            existing_hashes = {}
        ...
        for item in abs_folder_path.rglob('*'):
            ...
            if file_ext not in ACCEPTED_FILE_TYPES:
                progress_tracker.update(success=False, error_msg=f"Unsupported type: {file_ext}")
                continue
            
            try:
                # Get file metadata
                stat = item.stat()
                mod_time_iso = dt.fromtimestamp(stat.st_mtime, tz=timezone.utc).isoformat()
                acc_time_iso = dt.fromtimestamp(stat.st_atime, tz=timezone.utc).isoformat()
                
                # Generate hashes with progress indication
                hashed_path = hash_filepath(file_path_str)
                data_hash = generate_data_hash(file_path_str)
                
                if not hashed_path or not data_hash:
                    progress_tracker.update(success=False, error_msg="Hash generation failed")
                    continue
                
                # Check for content duplicates
                is_copy = 'Y' if data_hash in existing_hashes else 'N'
                if is_copy == 'Y':
                    log_statement('info', f"{LOG_INS}:INFO>>Duplicate content detected: {item.name}", Path(__file__).stem)
                ...
                if is_copy == 'N':
                    existing_hashes[data_hash] = self._next_designation + len(new_files_to_add)
                
                progress_tracker.update(success=True)
                
            except Exception as e:
                progress_tracker.update(success=False, error_msg=str(e))
                log_statement('error', f"{LOG_INS}:ERROR>>Error processing {item}: {e}", Path(__file__).stem, True)
        
        # Finish progress tracking
        progress_tracker.finish()
        
        # Append new files to repository
        if new_files_to_add:
            print(f"\nAdding {len(new_files_to_add)} new files to repository...")
            try:
                def row_generator():
                ...
                log_statement('info', f"{LOG_INS}:INFO>>Successfully added {len(new_files_to_add)} files", Path(__file__).stem)
                print(f"✓ Successfully added {len(new_files_to_add)} files to repository")
                
            except Exception as e:
                log_statement('error', f"{LOG_INS}:ERROR>>Failed to append files: {e}", Path(__file__).stem, True)
                print(f"✗ Failed to add files to repository: {e}")
        else:
            print("\nNo new files to add.")
        ...
        stats = progress_tracker.get_statistics()
        print(f"\nScan Statistics:")
        print(f"  Total files scanned: {stats['total_items']}")
        print(f"  Successfully processed: {stats['processed_items'] - stats['failed_items']}")
        print(f"  Failed/Skipped: {stats['failed_items']}")
        print(f"  Processing rate: {stats['process_rate']:.1f} files/s")
        print(f"  Peak memory usage: {max(r['memory_mb'] for r in stats.get('resources', [{'memory_mb': 0}]))} MB")
