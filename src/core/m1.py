...
try:
    ...
    def safe_operation(name, func): 
        try: return {"status": "success", "result": func()}
...
class DataProcessingConfig:
    """Configuration for data processing operations, extends RepoHandlerConfig"""
    repo_path: Path = field(default_factory=lambda: Path.cwd())
    output_directory: Path = field(default_factory=lambda: Path.cwd() / "processed_data")
    max_workers: int = 4
            ...
            if self._repo_manager is None and REPO_HANDLER_AVAILABLE:
                try:
                    self._repo_manager = RepoManager(
                        base_directory=self.config.repo_path.parent,
    ...
    def get_semantic_labeler(self) -> Optional[SemanticLabeler]:
        ...
        with self._lock:
            if self._semantic_labeler is None and PROCESSING_IMPORTS_AVAILABLE and self.config.enable_semantic_labeling:
                try:
                    self._semantic_labeler = SemanticLabeler(config=self.config.__dict__)
...
class DataProcessingContext:
    ...
    def __init__(self, config: Optional[DataProcessingConfig] = None):
        ...
        self.container = DataProcessingContainer(self.config)
        self.log_prefix = get_log_prefix(inspect.currentframe()) if REPO_HANDLER_AVAILABLE else LOG_INS
        ...
        log_statement('info', f"{self.log_prefix}:INFO>>DataProcessingContext initialized", Path(__file__).stem)
        ...
        try:
            if PROJECT_IMPORTS_AVAILABLE:
                ...
                return DataProcessingConfig(
                    repo_path=Path(config_dict.get('repo_path', Path.cwd())),
                    output_directory=Path(config_dict.get('output_directory', Path.cwd() / "processed_data")),
    ...
    def set_repository(self, repo_id: str, repo: RepoHandler) -> None:
        """Set current repository"""
        self.current_repo_id = repo_id
        self.current_repo_path = repo.config.repo_path
        ...
        log_statement('info', f"{self.log_prefix}:INFO>>Repository set: {repo_id} ({self.file_count} files)", 
                     Path(__file__).stem)
...
class RepositoryOperations:
    ...
    def setup_repository(self, directory_path: Union[str, Path]) -> OperationResult:
        """
        Set up a repository for the given directory path.
        Replaces the monolithic set_data_directory() function.
        """
        def _do_setup():
            # Validate and resolve path
            target_path = self._validate_directory_path(directory_path)
            if not target_path:
                raise ValueError(f"Invalid directory path: {directory_path}")
            
            # Generate repository ID
            repo_id = self._generate_repo_id(target_path)
            ...
            repo = self._get_or_create_repository(repo_manager, repo_id, target_path)
            
            # Scan and update repository
            scan_result = self._scan_and_update_repository(repo, target_path)
            
            # Set as current repository
            self.context.set_repository(repo_id, repo)
            ...
            result = {
                ...
                'files_tracked': scan_result.get('total_files', 0)
            }
            
            log_statement('info', f"{self.log_prefix}:INFO>>Repository setup completed for {target_path}", 
                         Path(__file__).stem)
            return result
    ...
    def _generate_repo_id(self, path: Path) -> str:
        """Generate a unique repository ID for the given path"""
    ...
    def _get_or_create_repository(self, repo_manager: RepoManager, repo_id: str, repo_path: Path) -> RepoHandler:
        ...
        create_result = repo_manager.create_repository(
            repo_id=repo_id,
            repo_path=repo_path,
            config_overrides=self.context.config.to_repo_handler_config().__dict__
        )
        
        if create_result['status'] != OperationStatus.SUCCESS:
            raise RuntimeError(f"Failed to create repository: {create_result.get('error')}")
        
        log_statement('info', f"{self.log_prefix}:INFO>>Created new repository: {repo_id}", 
                     Path(__file__).stem)
        return create_result['result']['repository']
    ...
    def _scan_and_update_repository(self, repo: RepoHandler, target_path: Path) -> Dict[str, Any]:
        ...
        try:
            # Use RepoHandler's built-in scanning
...
class DataDirectorySetupCommand:
    """Command for setting up data directory (replaces set_data_directory function)"""
    ...
    def get_description(self) -> str:
        ...
        return "Set up and scan a data directory for processing"
    
    def execute(self) -> OperationResult:
        """Execute the data directory setup command"""
        def _do_execute():
            print("\n--- Set Data Directory ---")
            
            # Get directory path from user
            default_path = self.context.config.repo_path
            user_input = input(f"Enter the full path to the data directory (default: {default_path}): ").strip()
            ...
            print(f"Setting up repository for: {directory_path}")
            
            # Set up repository
            setup_result = self.repo_ops.setup_repository(directory_path)
            ...
            if setup_result['status'] == OperationStatus.SUCCESS:
                ...
                print(f"  Path: {result_data['repo_path']}")
                print(f"  Files tracked: {result_data['files_tracked']}")
        ...
        return safe_operation("execute_data_directory_setup", _do_execute)
...
class RepositoryStatusDisplay:
    ...
    def display_repository_status(self) -> None:
        ...
        try:
            ...
            print(f"\n--- Repository Status ---")
            print(f"Repository Path: {self.context.current_repo_path}")
...
class ProcessingPipeline(ABC):
    ...
    def process_batch(self, max_files: Optional[int] = None) -> OperationResult:
        ...
        def _do_process_batch():
            ...
            progress_id = f"{self.get_pipeline_name()}_{int(time.time())}"
            if self._progress_handler:
                self._progress_handler.save_progress(progress_id, {
                    'pipeline': self.get_pipeline_name(),
            ...
            for i, file_path in enumerate(files_to_process):
                try:
                    ...
                    if result['status'] == OperationStatus.SUCCESS:
                        ...
                        log_statement('warning', f"{self.log_prefix}:WARNING>>Failed to process {Path(file_path).name}: {error_msg}", 
                                     Path(__file__).stem)
                
                except Exception as e:
                    failed_count += 1
                    errors.append(f"{Path(file_path).name}: {str(e)}")
                    log_statement('error', f"{self.log_prefix}:ERROR>>Exception processing {Path(file_path).name}: {e}", 
                                 Path(__file__).stem, exc_info=True)
                
                # Update progress
                if self._progress_handler and (i + 1) % 10 == 0:
                    self._progress_handler.save_progress(progress_id, {
                        'pipeline': self.get_pipeline_name(),
                        'total_files': len(files_to_process),
            ...
            log_statement('info', f"{self.log_prefix}:INFO>>{self.get_pipeline_name()} batch completed: {processed_count} processed, {failed_count} failed", 
                         Path(__file__).stem)
            return result
...
class LinguisticProcessingPipeline(ProcessingPipeline):
    ...
    def get_files_to_process(self) -> List[str]:
        """Get files ready for linguistic processing"""
        try:
            # Get files with NEW or DISCOVERED status
            df = self.repo.get_dataframe()
            if df is None or df.empty:
                return []
            
            # Filter for files ready for processing
            processable_statuses = [ProcessingStatus.NEW.value, ProcessingStatus.DISCOVERED.value]
            processable_files = df[df['status'].isin(processable_statuses)]
            
            return processable_files['filepath'].tolist()
    ...
    def process_file(self, file_path: str) -> OperationResult:
        """Process a single file through linguistic pipeline"""
        def _do_process_file():
            file_path_obj = Path(file_path)
            
            # Update file status to processing
            status_result = self.repo.update_file_status(
                file_path_obj, 
                ProcessingStatus.LINGUISTIC_PROCESSING.value,
                change_description="Starting linguistic processing"
            )
            
            if status_result['status'] != OperationStatus.SUCCESS:
                raise RuntimeError(f"Failed to update file status: {status_result.get('error')}")
            
            try:
                # Get data processor
                data_processor = self._get_data_processor()
                if not data_processor:
                    raise RuntimeError("Data processor not available")
                
                # Process the file (this would call the actual DataProcessor)
                processed_result = self._process_with_data_processor(data_processor, file_path_obj)
                
                # Get semantic labeler and apply if available
                semantic_label = None
                if self.context.config.enable_semantic_labeling:
                    semantic_labeler = self._get_semantic_labeler()
                    if semantic_labeler:
                        semantic_label = self._apply_semantic_labeling(semantic_labeler, processed_result)
                
                # Update file status and metadata
                update_result = self.repo.update_file_status(
                    file_path_obj,
                    ProcessingStatus.PROCESSED.value,
                    change_description="Linguistic processing completed"
                )
                
                if update_result['status'] != OperationStatus.SUCCESS:
                    log_statement('warning', f"{self.log_prefix}:WARNING>>Failed to update file status after processing: {update_result.get('error')}", 
                                 Path(__file__).stem)
                ...
                return {
                    ...
                    'processed_path': processed_result.get('output_path') if processed_result else None
                }
                ...
                self.repo.update_file_status(
                    ...
                    change_description=f"Linguistic processing failed: {str(e)}"
                )
                raise e
...
class TokenizationPipeline(ProcessingPipeline):
    ...
    def get_files_to_process(self) -> List[str]:
        ...
        try:
            ...
            # Filter for processed files ready for tokenization
            tokenizable_files = df[df['status'] == ProcessingStatus.PROCESSED.value]
...
class DataProcessingMenu:
    ...
    def _display_menu(self):
        ...
        if self.context.current_repo_path:
            print(f"Current Path: {self.context.current_repo_path.name}")
        
        print("-"*60)
        print("1. Set/Scan Data Directory")
